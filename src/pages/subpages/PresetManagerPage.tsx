  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      {showPresetDetail && (
        ...
        >
          <div 
            className="w-full max-w-md bg-[#1a1a1a] rounded-2xl overflow-hidden max-h-[90vh] overflow-y-auto"
              ...
              return (
                <>
                  <div className="aspect-video relative">
                    <img src={preset.coverPath} alt={preset.name} className="w-full h-full object-cover" />
                    ...
                    {/* Rating Badge */}
                    {detail && (
                      <div className="absolute bottom-4 left-4 flex items-center gap-2 px-3 py-2 rounded-xl bg-black/50 backdrop-blur-sm">
                        <Star size={16} className="text-yellow-400 fill-yellow-400" />
                        <span className="text-white font-bold">{detail.rating.toFixed(1)}</span>
                        <span className="text-white/50 text-sm">({detail.ratingCount} 评价)</span>
                      </div>
                    )}
                  </div>
                  <div className="p-4">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {preset.isHncs && <Crown size={16} className="text-[#FF6B35]" />}
                      {detail?.isPro && <Award size={16} className="text-yellow-400" />}
                    </div>
                    <p className="text-white/50 text-sm mb-3">{preset.author}</p>
                    ...
                    {detail && (
                      <div className="grid grid-cols-3 gap-3 mb-4 p-3 bg-white/5 rounded-xl">
                        <div className="text-center">
                          <p className="text-lg font-bold text-white">{(detail.downloads / 1000).toFixed(1)}k</p>
                          <p className="text-[10px] text-white/40">下载</p>
                        </div>
                        <div className="text-center">
                          <p className="text-lg font-bold text-white">{detail.rating.toFixed(1)}</p>
                          <p className="text-[10px] text-white/40">评分</p>
                        </div>
                        <div className="text-center">
                          <p className="text-lg font-bold text-white">{detail.ratingCount}</p>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4 p-3 bg-white/5 rounded-xl">
                      <div>
                        <p className="text-[10px] text-white/40">饱和度</p>
                        <p className="text-sm font-medium">{preset.saturation}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-white/40">对比度</p>
                        <p className="text-sm font-medium">{preset.contrast}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-white/40">亮度</p>
                        <p className="text-sm font-medium">{preset.brightness}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-white/40">锐度</p>
                        <p className="text-sm font-medium">{preset.sharpness}</p>
                      </div>
                    </div>
                    ...
                    {detail && detail.comments.length > 0 && (
                      <div className="mb-4">
                        <h3 className="text-white/70 text-sm font-medium mb-2">用户评价</h3>
                        <div className="space-y-2">
                          {detail.comments.slice(0, 2).map((comment) => (
                            <div key={comment.id} className="p-2 rounded-lg bg-white/5">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-white text-xs font-medium">{comment.user}</span>
                                <div className="flex">
                                  {[...Array(5)].map((_, i) => (
                                    <Star key={i} size={10} className={i < comment.rating ? 'text-yellow-400 fill-yellow-400' : 'text-white/20'} />
                                  ))}
                                </div>
                              </div>

...
class WatermarkEditorManagerTest {
    ...
    fun testVignetteCalculation() {
        ...
        val distance = kotlin.math.sqrt(
            kotlin.math.pow(pixelX - centerX, 2) + 
            kotlin.math.pow(pixelY - centerY, 2)
        )
...
class SmartWatermarkColorTest {
    ...
    fun testContrastEnhancement() {
        ...
        val contrast = kotlin.math.abs(watermarkColor - backgroundColor)
...
enum class HasselbladTemplate(val name: String) {
    CLASSIC("Classic"),
    MODERN("Modern"),
    MINIMAL("Minimal")
}

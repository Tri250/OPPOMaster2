...
class AIFineTuneViewModelTest {
    ...
    private val suggestedParamsFlow = MutableStateFlow<RenderParameters?>(null)
    ...
    fun setup() {
        ...
        whenever(aiManager.isProcessing).thenReturn(isProcessingFlow)
        whenever(aiManager.suggestedParams).thenReturn(suggestedParamsFlow)
        whenever(aiManager.errorState).thenReturn(errorStateFlow)
    ...
    fun `更新参数值更新当前参数`() = runTest {
        viewModel.updateParam("exposure", 10)
        
        assertEquals(10, viewModel.currentParams.value.exposure)
    }

    @Test
    fun `更新亮度参数`() = runTest {
        viewModel.updateParam("brightness", 20)
        
        assertEquals(20, viewModel.currentParams.value.brightness)
    }

    @Test
    fun `更新对比度参数`() = runTest {
        viewModel.updateParam("contrast", 15)
        
        assertEquals(15, viewModel.currentParams.value.contrast)
    }

    @Test
    fun `更新饱和度参数`() = runTest {
        viewModel.updateParam("saturation", 25)
        
        assertEquals(25, viewModel.currentParams.value.saturation)
    }

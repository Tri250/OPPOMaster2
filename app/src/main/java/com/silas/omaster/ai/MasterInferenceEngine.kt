...
class MasterInferenceEngine private constructor(context: Context) {
     ...
     * ST-AI-03: 预检模型文件存在性，避免加载损坏/缺失文件导致native crash
     */
    private fun initTFLiteModel() {
        try {
            // 先检查 assets 中模型文件是否存在
            val assetList = context.assets.list("models") ?: emptyArray()
            if (!assetList.contains("scene_classifier.tflite")) {
                android.util.Log.w(TAG, "MasterInferenceEngine: 模型文件不存在，跳过加载")

...
object XmpParser {
    ...
    fun parse(inputStream: InputStream): XmpParseResult {
        return try {
            // ST-PARSE-02: 安全限制 XMP 输入大小，防止超大/畸形文件导致 OOM
            // 不依赖 available()（某些流实现返回 0 或不准确），直接读取并截断
            val maxSize = 1024 * 1024 // 1MB
            val bytes = inputStream.use { it.readNBytes(maxSize + 1) }
            val limitedStream = if (bytes.size > maxSize) {
                Log.w(TAG, "XMP 文件超过 ${maxSize} bytes，仅读取前 $maxSize bytes")
                java.io.ByteArrayInputStream(bytes.copyOf(maxSize))
            } else {

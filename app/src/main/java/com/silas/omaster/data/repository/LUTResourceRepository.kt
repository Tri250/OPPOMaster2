...
object LUTResourceRepository {
    ...
    // 无效 URL 特征 - 用于过滤
    private val INVALID_URL_PATTERNS = listOf(
        "cubelut.cn",
        "download_restore.php",
    )

    /**
     * 验证下载 URL 是否有效
     * 有效的 URL 必须是非空、以 https 开头、且不包含已知无效域名/路径
     */
    fun isValidDownloadUrl(url: String): Boolean {
        if (url.isBlank()) return false
        val lower = url.lowercase()
        if (!lower.startsWith("https://")) return false
        if (INVALID_URL_PATTERNS.any { lower.contains(it) }) return false
        return true
    }

    /**
     * 获取所有有效资源（过滤掉无效 URL 的条目）
     */
    fun getValidResources(): List<LUTResource> {
        return RESOURCES.filter { isValidDownloadUrl(it.downloadUrl) }
    }

    // 2026年流行LUT资源库 - 对齐 Web 端 LUT_RESOURCES
    // 已移除无效 URL（cubelut.cn PHP 脚本）的条目，仅保留 CDN 直链资源
    ...
    fun getResources(category: String? = null): List<LUTResource> {
        val valid = getValidResources()
        if (category == null || category == "all") return valid
        return valid.filter { it.category == category }
    }

    fun searchResources(query: String): List<LUTResource> {
        val q = query.lowercase()
        return getValidResources().filter {
    ...
    fun getHotResources(): List<LUTResource> =
        getValidResources().filter { it.isHot }.sortedByDescending { it.downloads }

    fun getNewResources(): List<LUTResource> =
        getValidResources().filter { it.isNew }

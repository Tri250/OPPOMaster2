...
enum class LUTCategory(val key: String, val displayName: String, val icon: String) {
    ...
    HASSELBLAD("hasselblad", "哈苏大师", "👑");  // 新增：哈苏专属分类
...
enum class LUTFormat(val extension: String, val displayName: String) {
    ...
    companion object {
        fun fromExtension(ext: String): LUTFormat =
            entries.find { it.extension == ext.lowercase() } ?: CUBE
    }
}
...
enum class LUTSize(val value: Int, val displayName: String) {
    ...
    companion object {
        fun fromValue(value: Int): LUTSize =
            entries.find { it.value == value } ?: SIZE_33
    }
}

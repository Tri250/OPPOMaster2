 ...
 * 使用统一MasterLUT数据模型
 */
object MasterLUTRepository {
    
    val ALL_LUTS: List<MasterLUT> = listOf(
        // === 原生经典系列 ===
        MasterLUT(
            id = "classic-chrome",
            ...
            subCategory = "原生经典",
            tags = listOf("经典", "铬色", "复古", "低饱和"),
            suitableFor = listOf("街拍", "人文", "复古", "黑白"),
            ...
            mirrorUrls = listOf("https://cdn.jsdelivr.net/gh/hasselblad/luts/cc_33.cube"),
            ...
            compatibleSoftware = listOf("DaVinci Resolve", "Premiere Pro", "Final Cut Pro", "LumaFusion", "CapCut")
        ),
        MasterLUT(
            id = "neutral-color",
            ...
            category = LUTCategory.HASSELBLAD,
            subCategory = "原生经典",
            tags = listOf("中性", "自然", "还原", "日常"),
            suitableFor = listOf("风景", "建筑", "日常", "旅行"),
            format = LUTFormat.CUBE,
            size = LUTSize.SIZE_33,
            fileSize = 11264,
            ...
            minAppVersion = "1.0",
            compatibleSoftware = listOf("DaVinci Resolve", "Premiere Pro", "Final Cut Pro")
        ),
        MasterLUT(
            id = "natural-hue",
            name = "Natural Hue (NH)",
            ...
            category = LUTCategory.HASSELBLAD,
            subCategory = "原生经典",
            tags = listOf("自然", "色调", "肤色", "人像"),
            suitableFor = listOf("人像", "婚礼", "写真", "肖像"),
            format = LUTFormat.CUBE,
            size = LUTSize.SIZE_33,
            fileSize = 13312,
            ...
            generatedParams = LUTParams(
                ...
                contrast = 0.05f,
                brightness = 0f,
                colorTemperature = 0.1f,
                tint = 0f,
                highlightRolloff = 0.05f,
                shadowLift = 0f,
                skinProtection = true
            ),
            version = 1,
        ...
        MasterLUT(
            ...
            category = LUTCategory.HASSELBLAD,
            subCategory = "情绪表达",
            tags = listOf("富士", "反转片", "鲜艳", "风景"),
            suitableFor = listOf("风景", "自然", "旅行", "户外"),
            format = LUTFormat.CUBE,
            size = LUTSize.SIZE_33,
            fileSize = 14336,
        ...
        MasterLUT(
            ...
            category = LUTCategory.HASSELBLAD,
            subCategory = "结构时间",
            tags = listOf("夜景", "电影", "城市", "钨丝灯"),
            suitableFor = listOf("夜景", "城市", "室内", "电影"),
            format = LUTFormat.CUBE,
            size = LUTSize.SIZE_33,
            fileSize = 10240,
        ...
        MasterLUT(
            ...
            category = LUTCategory.HASSELBLAD,
            subCategory = "结构时间",
            tags = listOf("黑白", "颗粒", "纪实", "人文"),
            suitableFor = listOf("黑白", "人文", "纪实", "街拍"),
            format = LUTFormat.CUBE,
            size = LUTSize.SIZE_33,
            fileSize = 8192,
        ...
        MasterLUT(
            ...
            category = LUTCategory.HASSELBLAD,
            subCategory = "数字记忆",
            tags = listOf("数码", "暖调", "复古", "怀旧"),
            suitableFor = listOf("复古", "怀旧", "日常", "Vlog"),
            format = LUTFormat.CUBE,
            size = LUTSize.SIZE_33,
            fileSize = 8192,
        ...
        MasterLUT(
            ...
            category = LUTCategory.HASSELBLAD,
            subCategory = "数字记忆",
            tags = listOf("数码", "冷调", "清透", "现代"),
            suitableFor = listOf("现代", "清透", "科技", "开箱"),
            format = LUTFormat.CUBE,
            size = LUTSize.SIZE_33,
            fileSize = 9216,
     ...
     * 按分类获取LUT
     */
    fun getByCategory(category: LUTCategory): List<MasterLUT> {
        return if (category == LUTCategory.ALL) {
            ALL_LUTS
        } else {
            ALL_LUTS.filter { it.category == category }
        }
    }
     ...
     * 按子分类获取LUT
     */
    fun getBySubCategory(subCategory: String): List<MasterLUT> {
        return ALL_LUTS.filter { it.subCategory == subCategory }
    }
    
    /**
     * 按哈苏系列获取LUT
     */
    fun getByCollection(collection: String): List<MasterLUT> {
        return ALL_LUTS.filter { it.hasselbladCollection == collection }
    }
    ...
    fun getHotLUTs(): List<MasterLUT> {
        return ALL_LUTS.filter { it.isHot }.sortedByDescending { it.downloads }
    }
    ...
    fun getFeaturedLUTs(): List<MasterLUT> {
        return ALL_LUTS.filter { it.isFeatured }
    }
    ...
    fun getHncsCertifiedLUTs(): List<MasterLUT> {
        return ALL_LUTS.filter { it.isHncsCertified }
    }
    
    /**
     * 搜索LUT
     */
    fun search(query: String): List<MasterLUT> {
        val q = query.lowercase()
        return ALL_LUTS.filter { lut ->
    ...
    fun formatFileSize(bytes: Long): String {
        return when {
            ...
            else -> "${(bytes / (1024 * 1024)).toFloat()} MB"
        }
    }
     ...
     * 格式化下载数
     */
    fun formatDownloads(count: Long): String {
        return when {
            count >= 10000 -> "${(count / 10000).toFloat()}万"
            else -> count.toString()
        }
    }
}